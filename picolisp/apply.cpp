...
any apply(any ex, any foo, be cf, int n, cell *p) {
   while (!isNum(foo)) {
      if (isCell(foo)) {
         ...
         f.bnd = (sv*)malloc( (length(x)+2)  * sizeof(sv));
      ...
      if (val(foo) == val(Meth)) {
         ...
         if (expr = method(o)) {
            ...
            f.bnd = (sv*)malloc( (length(x = car(expr))+3)  * sizeof(sv));
...
any doPass(any ex) {
   ...
   vec c(length(cdr(x = cdr(ex))) + (Env.next > 0 ? Env.next : 0));
...
any doMaplist(any ex) {
   ...
   if (isCell(x = cdr(x))) {
      ...
      if (!isCell(data(c[0])))
         return Pop(res);
      data(res) = x = cons(apply(ex, data(foo), NO, n, &c[0]), Nil);
      while (isCell(data(c[0]) = cdr(data(c[0])))) {
         for (i = 1; i < n; ++i)
            data(c[i]) = cdr(data(c[i]));
         cdr(x) = cons(apply(ex, data(foo), NO, n, &c[0]), Nil);
         x = cdr(x);
      }
   }
   return Pop(res);
...
any doMapcar(any ex) {
   ...
   if (isCell(x = cdr(x))) {
      ...
      if (!isCell(data(c[0])))
         return Pop(res);
      data(res) = x = cons(apply(ex, data(foo), YES, n, &c[0]), Nil);
      while (isCell(data(c[0]) = cdr(data(c[0])))) {
         for (i = 1; i < n; ++i)
            data(c[i]) = cdr(data(c[i]));
         cdr(x) = cons(apply(ex, data(foo), YES, n, &c[0]), Nil);
         x = cdr(x);
      }
   }
   return Pop(res);
...
any doMapcon(any ex) {
   ...
   if (isCell(x = cdr(x))) {
      ...
      if (!isCell(data(c[0])))
         return Pop(res);
      while (!isCell(x = apply(ex, data(foo), NO, n, &c[0]))) {
      ...
      while (isCell(data(c[0]) = cdr(data(c[0])))) {
         ...
         while (isCell(cdr(x)))
            x = cdr(x);
         cdr(x) = apply(ex, data(foo), NO, n, &c[0]);
      }
   }
   return Pop(res);
}
...
any doMapcan(any ex) {
   ...
   if (isCell(x = cdr(x))) {
      ...
      if (!isCell(data(c[0])))
         return Pop(res);
      while (!isCell(x = apply(ex, data(foo), YES, n, &c[0]))) {
         if (!isCell(data(c[0]) = cdr(data(c[0]))))
            return Pop(res);
         for (i = 1; i < n; ++i)
            data(c[i]) = cdr(data(c[i]));
      ...
      while (isCell(data(c[0]) = cdr(data(c[0])))) {
         ...
         while (isCell(cdr(x)))
            x = cdr(x);
         cdr(x) = apply(ex, data(foo), YES, n, &c[0]);
      }
   }
   return Pop(res);
}
...
any doFilter(any ex) {
   ...
   if (isCell(x = cdr(x))) {
      ...
      if (!isCell(data(c[0])))
         return Pop(res);
      while (isNil(apply(ex, data(foo), YES, n, &c[0]))) {
         if (!isCell(data(c[0]) = cdr(data(c[0]))))
            return Pop(res);
         for (i = 1; i < n; ++i)
            data(c[i]) = cdr(data(c[i]));
      ...
      while (isCell(data(c[0]) = cdr(data(c[0])))) {
         for (i = 1; i < n; ++i)
            data(c[i]) = cdr(data(c[i]));
         if (!isNil(apply(ex, data(foo), YES, n, &c[0])))
...
any doExtract(any ex) {
   ...
   if (isCell(x = cdr(x))) {
      ...
      if (!isCell(data(c[0])))
         return Pop(res);
      while (isNil(y = apply(ex, data(foo), YES, n, &c[0]))) {
         if (!isCell(data(c[0]) = cdr(data(c[0]))))
            return Pop(res);
         for (i = 1; i < n; ++i)
            data(c[i]) = cdr(data(c[i]));
      ...
      while (isCell(data(c[0]) = cdr(data(c[0])))) {
         for (i = 1; i < n; ++i)
            data(c[i]) = cdr(data(c[i]));
         if (!isNil(y = apply(ex, data(foo), YES, n, &c[0])))
            x = cdr(x) = cons(y, Nil);
      }
   }
   return Pop(res);
...
any doSeek(any ex) {
   ...
   if (isCell(x = cdr(x))) {
      ...
      while (isCell(x = cdr(x)));
      while (isCell(data(c[0]))) {
         if (!isNil(x = apply(ex, data(foo), NO, n, &c[0]))) {
            drop(foo);
            val(At2) = x;
            return data(c[0]);
         }
...
any doFind(any ex) {
   ...
   if (isCell(x = cdr(x))) {
      ...
      while (isCell(x = cdr(x)));
      while (isCell(data(c[0]))) {
         if (!isNil(x = apply(ex, data(foo), YES, n, &c[0]))) {
            drop(foo);
            val(At2) = x;
            return car(data(c[0]));
         }
...
any doSum(any ex) {
   ...
   if (isCell(x = cdr(x))) {
      ...
      while (isCell(x = cdr(x)));
      while (isCell(data(c[0]))) {
         if (isNum(x = apply(ex, data(foo), YES, n, &c[0])))
            res += unBox(x);
         for (i = 0; i < n; ++i)
            data(c[i]) = cdr(data(c[i]));
      }
...
any doMaxi(any ex) {
   ...
   if (isCell(x = cdr(x))) {
      ...
      while (isCell(x = cdr(x)));
      while (isCell(data(c[0]))) {
         if (compare(x = apply(ex, data(foo), YES, n, &c[0]), data(val)) > 0)
...
any doBy(any ex) {
   ...
   if (isCell(x = cdr(x))) {
      ...
      if (!isCell(data(c[0])))
         return Pop(res);
      data(res) = x = cons(cons(apply(ex, data(foo1), YES, n, &c[0]), car(data(c[0]))), Nil);
      while (isCell(data(c[0]) = cdr(data(c[0])))) {
         for (i = 1; i < n; ++i)
            data(c[i]) = cdr(data(c[i]));
         cdr(x) = cons(cons(apply(ex, data(foo1), YES, n, &c[0]), car(data(c[0]))), Nil);
         x = cdr(x);
      }
